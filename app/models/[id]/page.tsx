...
import { fmt, fmt0, pct, daysAgo, today } from "@/lib/format";
import { pickPrevRange } from "@/lib/compare";
import type { CompareConfig, CompareMode, ModelStat, Snapshot } from "@/lib/types";
import { CHART_TOOLTIP, ageColor } from "@/lib/chartTheme";
...
export default function ModelPage() {
  ...
  // Recompute derived compare range when main range changes.
  useEffect(() => {
    setCompare((c) => {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-6">
      <header className="mb-4 flex flex-wrap items-end justify-between gap-3 border-b border-line pb-4">
        <div>
          <Link href="/" className="text-xs text-ink3 hover:text-ink">
            ← back to dashboard
          </Link>
